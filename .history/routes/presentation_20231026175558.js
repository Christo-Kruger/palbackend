...
router.get("/", async (req, res) => {
  try {
    ...
    //const userId = req.user._id; // Assuming you have access to user's id from req.user
    ...
    const extractedPresentations = presentations.map((presentation) => ({
      ...
      timeSlots: presentation.timeSlots.map((timeSlot) => ({
        ...
        // Include only the count of attendees, not the data
        attendeeCount: timeSlot.attendees ? timeSlot.attendees.length : 0, // Added check for undefined
        isCurrentUserAnAttendee: timeSlot.attendees ? timeSlot.attendees.includes(userId) : false, // Added
      })),
    }));
