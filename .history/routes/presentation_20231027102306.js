...
router.get("/", async (req, res) => {
  try {
    ...
    const childAgeGroup = req.query.ageGroup;
    const userID = req.query.userID; // Assuming you get userID from query
    ...
    if (presentations.length === 0) {
      return res
        .status(404)
        .json({ message: "No matching presentations found" });
    }

    const extractedPresentations = presentations.map((presentation) => ({
      ...
      timeSlots: presentation.timeSlots.map((timeSlot) => ({
        ...
        attendeeCount: timeSlot.attendees.length,
        // Check if userID is in the list of attendees for the timeSlot
        isUserAttending: timeSlot.attendees.includes(userID),
      })),
    }));

    res.json(extractedPresentations);
    console.log(extractedPresentations);
  } catch (err) {
...
router.get("/allAttendeesInTimeSlots", auth, async (req, res) => {
  try {
    ...
    const limit = parseInt(req.query.limit) || 100; // Default limit is 100
...
// Slice the array to only include up to `limit` attendees
const limitedAttendeesInTimeSlots = allAttendeesInTimeSlots.slice(0, limit);
    res.json({ data: allAttendeesInTimeSlots, page: page, limit: limit });
  } catch (err) {
    console.error("Error processing request:", err);
    res.status(500).json({ message: "Internal Server Error" });
  }
