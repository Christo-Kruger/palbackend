  ...
  try {
    ...
    const extractedPresentations = presentations.map((presentation) => ({
      ...
      timeSlots: presentation.timeSlots.map((timeSlot) => ({
        ...
        attendeeCount: timeSlot.attendees ? timeSlot.attendees.length : 0,
        isCurrentUserAnAttendee: userId ? (timeSlot.attendees ? timeSlot.attendees.includes(userId) : false) : null,
      })),
    }));
